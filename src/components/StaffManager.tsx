...
export default function StaffManager({ restaurantCode, restaurantName, pin, payPeriodStart, onPayPeriodStartSaved }: Props) {
  ...
  const openEditDialog = (s: Staff) => {
    ...
    setFormPin(s.pin); setFormActive(s.is_active);
    setFormError('');
    setDialogOpen(true);
  };
  ...
  const entriesByStaff = entries.reduce<Record<string, { name: string; wage: number; entries: TimeEntryRow[] }>>((acc, e) => {
    if (!acc[e.staff_id]) acc[e.staff_id] = { name: e.staff_name, wage: e.hourly_wage, entries: [] };
      ...
      {subTab === 'staff' && (
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-semibold">Staff Members ({staffList.length})</h2>
      ...
      {subTab === 'payroll' && (
        <div className="space-y-4">
          ...
          {entriesLoading ? (
            ...
            <>
              {Object.entries(entriesByStaff).map(([staffId, { name, wage, entries: staffEntries }]) => {
                ...
                const totalPayHours = staffEntries.reduce((sum, e) => {
                  ...
                  return sum + calcWorkedHours(e.clock_in, e.clock_out);
                }, 0);
                const grossPay = Math.round(totalPayHours * wage); // wage in cents → result in cents
                  ...
                  <Card key={staffId}>
                    <CardHeader className="pb-2">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-xs text-muted-foreground">{formatMoney(wage)}/hr</span>
                      </div>
                    </CardHeader>
                    <CardContent>
                              ...
                              .map((e) => {
                                ...
                                return (
                                  <tr key={e.id} className="border-b border-border/50">
                                    ...
                                    <td className="py-1.5 pr-3">{e.clock_out ? formatTime(e.clock_out) : <span className="text-amber-500">Active</span>}</td>
                                    <td className="py-1.5 pr-3">{e.clock_out ? formatHours(worked) : '—'}</td>
                                    ...
                                    <td className="py-1.5 w-8 no-print">
                                      {!isEditing && e.clock_out && (
                                        <button
                                          onClick={() => { setEditingEntry(e.id); setEditPayHours((e.pay_hours ?? worked).toFixed(2)); }}
                                          className="text-muted-foreground hover:text-foreground transition-colors"
                                          title="Edit pay hours"
                                        >
                                          <Pencil size={12} />
                                        </button>
                                      )}
          ...
          <div className="hidden print:block" id="pay-stub-print">
            ...
            {Object.entries(entriesByStaff).map(([staffId, { name, wage, entries: staffEntries }]) => {
              ...
              const grossPay = Math.round(totalPayHours * wage);
                  ...
                  <table className="w-full text-sm border-collapse">
                    ...
                    <tbody>
                      {staffEntries
                        .sort((a, b) => new Date(a.clock_in).getTime() - new Date(b.clock_in).getTime())
                        .map((e) => {
                          const payH = e.pay_hours ?? calcWorkedHours(e.clock_in, e.clock_out);
                          return (
